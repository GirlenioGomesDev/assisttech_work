...
const navigation = [
  ...
  { name: 'Notificacoes', href: '/notificacoes', icon: Bell, roles: ['admin','atendente','tecnico','financeiro'] },
  ...
  { name: 'Seguranca', href: '/seguranca', icon: ShieldCheck, roles: ['admin'] },
  { name: 'Configuracoes', href: '/configuracoes', icon: Settings, roles: ['admin'] },
  { name: 'Backup', href: '/backup', icon: DatabaseBackup, roles: ['admin'] },
];
...
export function MainLayout() {
  ...
  <div className="lg:pl-64"><div className="sticky top-0 z-30 flex items-center gap-3 h-16 px-6 bg-white border-b border-gray-200 dark:bg-gray-900 dark:border-gray-800"><Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setSidebarOpen(true)}><Menu className="h-5 w-5" /></Button><div className="min-w-0 flex-1 lg:ml-0 ml-1"><h2 className="truncate text-lg text-gray-900 dark:text-gray-100">{nav.find(item => item.href === location.pathname)?.name || 'AssistTech'}</h2></div><GlobalSearch /><Button type="button" variant="outline" size="icon" onClick={toggleTheme} title={theme === 'dark' ? 'Usar modo claro' : 'Usar modo escuro'} aria-label={theme === 'dark' ? 'Usar modo claro' : 'Usar modo escuro'}>{theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button></div><main className="p-6"><Outlet /></main></div></div>);
}
